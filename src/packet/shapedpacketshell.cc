...
ShapedPacketShell<FerryQueueType>::ShapedPacketShell( const std::string & device_prefix, char ** const user_environment )
    : user_environment_( user_environment ),
      ...
      egress_name( device_prefix + "-" + to_string( getpid() ) ),
      ...
      event_loop_()
{
    ...
    if ( environ != nullptr ) {
        throw runtime_error( "ShapedPacketShell: environment was not cleared" );
    }
...
void ShapedPacketShell<FerryQueueType>::start_uplink( const string & shell_prefix,
                                                ...
                                                Targs&&... Fargs )
{
    ...
    */
    ...
    /* Fork */
    event_loop_.add_special_child_process( 77, "Shapedpacketshell", [&]() {
            ...
            add_tc_rule("ingress", delay_ms, uplink_kbits_s);
...
template <class FerryQueueType>
template <typename... Targs>
void ShapedPacketShell<FerryQueueType>::start_downlink(const uint64_t delay_ms,
                                                const uint64_t ,
                                                const uint64_t downlink,
    Targs&&... Fargs )
{
    /* g++ bug 55914 makes this hard before version 4.9 */
    BindWorkAround::bind<FerryQueueType, Targs&&...> ferry_maker( forward<Targs>( Fargs )... );
    ...
    */
    ...
    event_loop_.add_special_child_process( 77, "downlink", [&] () {
            add_tc_rule(egress_name, delay_ms, downlink);
            run({"/sbin/ifconfig"});
...
template <class FerryQueueType>
int ShapedPacketShell<FerryQueueType>::wait_for_exit( void )
{
    return event_loop_.loop();
}

template <class FerryQueueType>
int ShapedPacketShell<FerryQueueType>::Ferry::loop( FerryQueueType & ferry_queue,
                                              FileDescriptor & tun,
...
template <class FerryQueueType>
Address ShapedPacketShell<FerryQueueType>::get_mahimahi_base( void ) const
{
