...
:dirname("/tmp/nonexistent")
{
    ...
    const std::string CA_PATH = mahimahi_root + "/mahimahica/";
    char tpl[] = "/tmp/tmp_ca.XXXXXX";
    this->dirname = mkdtemp(tpl);

    copyFile(CA_PATH+"/index.txt",this->dirname+"/index.txt");
    copyFile(CA_PATH+"/cacert.pem",this->dirname+"/cacert.pem");
    copyFile(CA_PATH+"/cakey.pem",this->dirname+"/cakey.pem");
...
CAEnvironment::~CAEnvironment()
{
    ...
    while ( (next_file = readdir(theFolder)) != NULL )
    {
        ...
        sprintf(filepath, "%s/%s", this->dirname.c_str(), next_file->d_name);
        remove(filepath);
    }
    closedir(theFolder);
